...
public class Erosion {
    ...
    public Erosion() {
        erodeSpeed = Constants.EROSION;
        depositSpeed = Constants.DEPOSITION;
        modifier = Modifier.range(Constants.GROUND, Constants.ground(15));
    }

    public void apply(Filterable map, int seedX, int seedZ, int iterations) {
        if (erosionBrushIndices.length != map.getBacking().length) {
